  ...
  useEffect(() => {
    const handleAuthSuccess = async () => {
      try {
        ...
        if (!token) {
          setError(t('authSuccess.errors.noToken'));
          return;
        }
        ...
        setError(t('authSuccess.errors.failed'));
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#F9F7F7] dark:bg-slate-950">
        <div className="text-center">
          ...
          <p className="mt-4 text-[#112D4E] dark:text-sky-200">
            {t('authSuccess.loading')}
          </p>
        </div>
      </div>
    );
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#F9F7F7] dark:bg-slate-950">
        <div className="max-w-md w-full bg-white dark:bg-slate-900 p-8 rounded-xl shadow-lg border border-[#DBE2EF] dark:border-slate-700 text-center">
          ...
          <h2 className="text-2xl font-bold text-[#112D4E] dark:text-sky-200 mb-2">
            {t('authSuccess.errorTitle')}
          </h2>
