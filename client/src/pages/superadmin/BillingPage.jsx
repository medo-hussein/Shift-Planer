        ...
        <div className="p-6 md:p-10 min-h-screen bg-gray-50 dark:bg-slate-900">
            ...
            <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 mb-10">
                <div className="flex flex-col md:flex-row justify-between items-center gap-6">
                    <div>
                        <h2 className="text-sm font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-1">Current Plan</h2>
                        <div className="flex items-center gap-3">
                            <h3 className="text-2xl font-bold text-gray-900 dark:text-white">{currentPlanName}</h3>
                            <StatusBadge status={status} />
                        </div>
                        <p className="text-gray-600 dark:text-gray-300 mt-1">
                            {status === "active" ? `Renews on: ${expiresAt}` : "No active subscription"}
                        </p>
                    </div>
                </div>
            </div>
                ...
                {loadingPlans ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        {plans.map((plan) => {
                            ...
                            return (
                                <div key={plan._id} className={`relative rounded-xl p-6 transition-all duration-300 flex flex-col border ${isCurrent ? "border-sky-500 bg-sky-50 dark:bg-slate-800/50 ring-1 ring-sky-500" : "bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 hover:shadow-lg"}`}>
                                    {isCurrent && (
                                    ...
                                    <div className="text-center mb-6 mt-2">
                                        ...
                                        <div className="flex items-baseline justify-center mt-2">
                                            <span className="text-3xl font-bold text-gray-900 dark:text-white">{plan.price} <span className="text-sm">EGP</span></span>
                                        </div>
                                    </div>
                                    ...
                                    <button
                                        ...
                                        className={`w-full py-2 px-4 rounded-lg font-medium transition-all flex items-center justify-center gap-2 ${isCurrent ? "bg-green-100 text-green-700 cursor-not-allowed" : "bg-gray-900 text-white hover:bg-black dark:bg-sky-600 dark:hover:bg-sky-700"}`}
                                    >
                                        {processing === plan._id ? <Loader2 className="w-4 h-4 animate-spin" /> : isCurrent ? "Active Plan" : "Upgrade"}
                                    </button>
                                </div>
                            );
                        })}
                    ...
                    {loadingHistory ? (
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full text-left text-sm">
                                <thead className="bg-gray-50 dark:bg-slate-700/50 text-gray-600 dark:text-gray-300 uppercase tracking-wider text-xs">
                                    <tr>
                                    ...
                                    {history.map((record) => (
                                        <tr key={record._id} className="hover:bg-gray-50 dark:hover:bg-slate-700/50 transition-colors">
                                            <td className="p-4 text-gray-900 dark:text-gray-200">
                                                ...
                                                <div className="text-xs text-gray-400">{new Date(record.payment_date).toLocaleTimeString()}</div>
                                            </td>
                                            <td className="p-4 text-gray-600 dark:text-gray-300">
                                                <div className="font-medium text-gray-900 dark:text-white">Subscription: {record.plan}</div>
                                                <div className="text-xs text-gray-400">Transaction ID: {record.transaction_id}</div>
                                            </td>
