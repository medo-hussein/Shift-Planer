...
// NOTE: This component assumes Tailwind is configured with `darkMode: 'class'`.
// The ThemeToggle below toggles a `dark` class on <html> and saves the preference in localStorage.
...
export default function EmployeeDashboard() {
  ...
  if (!data) return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-slate-900">
      <div className="text-slate-400 dark:text-slate-300">Loading dashboard...</div>
    </div>
  );
    ...
    <div className="p-6 min-h-screen bg-gray-50 dark:bg-slate-900 transition-colors duration-200">
      <div className="max-w-7xl mx-auto">
        <header className="flex items-center justify-between mb-6">
          <h1 className="text-2xl font-extrabold text-slate-900 dark:text-slate-100">My Dashboard</h1>
        </header>
        ...
        <div className="bg-white dark:bg-slate-800/60 dark:border-slate-700 p-6 rounded-2xl shadow-md border border-slate-100 mb-6 flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="flex items-center gap-4">
            <div className={`p-4 rounded-full ${today.clocked_in ? "bg-emerald-100 text-emerald-600" : "bg-slate-100 text-slate-500"}`}>
              <Clock size={32} />
            </div>
            <div>
          ...
          {today.current_shift ? (
            <div className="bg-linear-to-br from-blue-50 to-blue-100 dark:from-blue-900 dark:to-blue-800 px-6 py-3 rounded-xl border border-blue-100 dark:border-blue-700 text-center">
              <p className="text-xs text-blue-600 dark:text-blue-200 uppercase font-bold mb-1">Current Shift</p>
              <p className="text-blue-900 dark:text-blue-50 font-bold">
                {formatTime(today.current_shift.start_date_time)} - {formatTime(today.current_shift.end_date_time)}
              </p>
            </div>
          ) : (
            <div className="text-slate-400 dark:text-slate-300 text-sm flex items-center gap-2">
              <AlertCircle size={16} /> <span>No active shift right now</span>
            </div>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <StatCard title="Weekly Hours" value={`${(weekly.total_hours || 0).toFixed(1)}h`} icon={<Clock />} color="blue" />
          <StatCard title="Overtime" value={`${(weekly.overtime || 0).toFixed(1)}h`} icon={<TrendingUp />} color="orange" />
          <StatCard title="Days Present" value={`${weekly.present_days || 0} / ${weekly.total_days || 0}`} icon={<Calendar />} color="emerald" />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 bg-white dark:bg-slate-800/60 p-6 rounded-2xl shadow-md border border-slate-100 dark:border-slate-700">
            <h3 className="font-bold text-slate-800 dark:text-slate-100 mb-4 flex items-center gap-2">
              <Calendar size={20} className="text-blue-600 dark:text-blue-300"/> Upcoming Shifts
            </h3>
            <div className="space-y-4">
              {upcoming.shifts && upcoming.shifts.length > 0 ? upcoming.shifts.map(shift => (
                <div key={shift._id} className="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-900/40 rounded-xl border border-slate-100 dark:border-slate-700">
                  <div className="flex items-center gap-4">
                    <div className="text-center bg-white dark:bg-slate-800 p-2 rounded-lg border border-slate-200 dark:border-slate-700 min-w-[60px]">
                      <p className="text-xs text-slate-500 dark:text-slate-300 uppercase">{new Date(shift.start_date_time).toLocaleDateString('en-US', {weekday: 'short'})}</p>
                      <p className="text-lg font-bold text-slate-800 dark:text-slate-100">{new Date(shift.start_date_time).getDate()}</p>
                    </div>
                    <div>
                      <p className="font-bold text-slate-700 dark:text-slate-100">{shift.title || "Regular Shift"}</p>
                      <p className="text-xs text-slate-500 dark:text-slate-300 flex items-center gap-1">
                        <Clock size={10} /> {formatTime(shift.start_date_time)} - {formatTime(shift.end_date_time)}
                      </p>
                    </div>
                  </div>
                ...
                <p className="text-slate-400 dark:text-slate-300 text-center py-4">No upcoming shifts scheduled.</p>
              )}
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-slate-800/60 p-6 rounded-2xl shadow-md border border-slate-100 dark:border-slate-700 h-fit">
            <h3 className="font-bold text-slate-800 dark:text-slate-100 mb-4 flex items-center gap-2">
              <Briefcase size={20} className="text-purple-600 dark:text-purple-300"/> My Team
            </h3>

            <div className="mb-4">
               <p className="text-xs text-slate-400 dark:text-slate-300 uppercase font-bold">Branch</p>
               <p className="text-lg font-medium text-slate-700 dark:text-slate-100 flex items-center gap-2">
                 <MapPin size={16} className="text-slate-400 dark:text-slate-300"/> {branch.name || "Main Branch"}
               </p>
            </div>

            {branch.admin && (
              <div className="pt-4 border-t border-slate-100 dark:border-slate-700">
                 <p className="text-xs text-slate-400 dark:text-slate-300 uppercase font-bold mb-2">Branch Manager</p>
...
function StatCard({ title, value, icon, color }) {
    ...
    return (
        <div className="bg-white dark:bg-slate-800/60 p-5 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 flex items-center gap-4">
            <div className={`p-3 rounded-xl ${colors[color]} flex items-center justify-center`}>{icon}</div>
            <div>
