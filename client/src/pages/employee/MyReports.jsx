...
export default function MyReports() {
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <h1 className="text-2xl font-bold text-slate-900 mb-6">My Performance Reports</h1>
      
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {reports.length > 0 ? reports.map(report => (
          <div key={report._id} className="bg-white p-5 rounded-xl shadow-sm border border-slate-200 hover:shadow-md transition">
            <div className="flex justify-between items-start mb-3">
              <div className="p-2 bg-blue-50 text-blue-600 rounded-lg">
                {report.type === 'attendance' ? <Clock size={20}/> : <FileText size={20}/>}
              </div>
              <button onClick={() => setSelectedReport(report)} className="text-sm text-blue-600 hover:underline flex items-center gap-1">
                <Eye size={14}/> View
              </button>
            </div>
          ...
          <div className="col-span-full text-center py-10 text-slate-400">No reports shared with you yet.</div>
        )}
      </div>
