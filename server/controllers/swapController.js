...
export const createSwapRequest = async (req, res) => {
  try {
    ...
    const shift = await Shift.findOne({ _id: shift_id, employee_id: requesterId });
    if (!shift) {
      return res.status(404).json({ success: false, message: "Shift not found or does not belong to you." });
    }
    ...
    return res.status(201).json({ success: true, message: "Swap request created", data: swapRequest });
  } catch (err) {
...
export const acceptSwapRequest = async (req, res) => {
    try {
        ...
        // 1. Check eligibility
        if (request.target_employee_id && request.target_employee_id.toString() !== userId.toString()) {
        ...
        if (!shiftToSwap) {
             return res.status(404).json({ success: false, message: "Shift associated with this request not found." });
        }
        ...
        if (myConflictingShift) {
            return res.status(400).json({ 
                success: false, 
                message: "You cannot accept this request because you already have a shift on this day." 
            });
        }
        ...
        return res.json({ success: true, message: "Request accepted. Waiting for manager approval." });
...
export const approveSwapRequest = async (req, res) => {
    try {
        ...
        if (!request || request.status !== 'accepted') {
            return res.status(400).json({ success: false, message: "Request not found or not in 'accepted' state." });
        }
        ...
        if (!originalShift) return res.status(404).json({ message: "Original shift not found" });
        ...
        // Notify Requester
        await createNotification({ ...notificationData, recipientId: request.requester_id });
        // Notify Target
        await createNotification({ ...notificationData, recipientId: request.target_employee_id });

        return res.json({ success: true, message: "Swap approved and schedule updated." });
...
export const getBranchSwapRequests = async (req, res) => {
    try {
        // Admin only
        if (!['admin', 'super_admin'].includes(req.user.role)) return res.status(403).json({ message: "Denied" });
