  ...
  try {
    ...
    if (userRole === "employee") {
      ...
      if (!approverId) {
        return res
          ...
          .json({
            success: false,
            message:
              "Employee must belong to a branch admin to submit a request.",
          });
      }
      ...
      if (!approverId) {
        return res
          ...
          .json({
            success: false,
            message: "Admin account is not linked to a Super Admin.",
          });
      }
    } else {
      return res
        ...
        .json({
          success: false,
          message: "Only employees and admins can submit leave requests.",
        });
    }
    ...
    if (approverId) {
      await createNotification({
        ...
        message: `${
          req.user.name
        } has requested a ${leave_type} leave from ${start.toLocaleDateString()} to ${end.toLocaleDateString()}.`,
        type: "info",
        link: "/time-off",
      });
    }
  ...
  try {
    ...
    if (!request) {
      return res
        ...
        .json({
          success: false,
          message: "Leave request not found or not authorized for approval.",
        });
    }
